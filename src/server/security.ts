...
export async function protectMutation(req:NextRequest){const cfg=getAuthConfig();if(!cfg)return 'Server konfiqurasiya edilməyib';const origin=req.headers.get('origin'),host=req.headers.get('host'),forwardedProto=req.headers.get('x-forwarded-proto')?.split(',')[0]?.trim(),expectedProtocol=forwardedProto||(process.env.NODE_ENV==='production'?'https':'http');let parsed:URL;try{parsed=new URL(origin||'')}catch{return 'Başqa mənbədən sorğuya icazə verilmir'}if(!host||parsed.host!==host||parsed.protocol!==expectedProtocol+':')return 'Başqa mənbədən sorğuya icazə verilmir';const session=decodeSession(req.cookies.get(COOKIE)?.value,cfg.sessionSecret);if(!session)return 'Giriş tələb olunur';if(!safeEqual(req.headers.get('x-csrf-token')||'',session.csrf))return 'Yanlış CSRF tokeni';return null}
