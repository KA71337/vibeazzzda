...
export function ProductImage({src,alt,priority=false,sizes,className='object-contain',onRatio}:ProductImageProps){
 ...
 if(!src||failed)return <div role="img" aria-label={`${alt}: ${t.imageUnavailable}`} className="absolute inset-0 grid place-items-center bg-gray-100 text-gray-400"><span className="flex flex-col items-center gap-2 text-center text-xs"><ImageOff aria-hidden="true"/><span>{t.imageUnavailable}</span></span></div>;
