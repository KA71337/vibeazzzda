...
export function Catalog({limit}:{limit?:number}){
 ...
 const controls=<><Select value={cat} onChange={setCat} options={catOptions} label={t.category} placeholder={t.allCategories} icon={<LayoutGrid size={17}/>}/><Select value={sort} onChange={setSort} options={sortOptions} label={t.sort} icon={<SlidersHorizontal size={17}/>}/><div className="grid grid-cols-2 gap-2"><label className="rounded-[1.15rem] bg-white px-4 py-2 shadow-sm"><span className="block text-[10px] font-bold uppercase tracking-wider text-gray-400">Min AZN</span><input value={min} onChange={e=>setMin(e.target.value)} type="number" inputMode="numeric" placeholder="0" aria-label={t.priceMin} className="mt-1 w-full bg-transparent text-sm font-semibold outline-none"/></label><label className="rounded-[1.15rem] bg-white px-4 py-2 shadow-sm"><span className="block text-[10px] font-bold uppercase tracking-wider text-gray-400">Max AZN</span><input value={max} onChange={e=>setMax(e.target.value)} type="number" inputMode="numeric" placeholder="∞" aria-label={t.priceMax} className="mt-1 w-full bg-transparent text-sm font-semibold outline-none"/></label></div></>;
 return <>
