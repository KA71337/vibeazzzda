...
export function ProductDetail({p}:{p:Product}){
 ...
 const addQuantity=()=>{for(let i=0;i<qty;i++)add(p)};
  ...
  <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1.18fr)_minmax(370px,.82fr)] lg:gap-10 xl:gap-14">
   <div data-gallery className="min-w-0">
    <div className="mx-auto w-full" style={{maxWidth}}>
     <motion.div data-gallery-frame key={img} initial={{opacity:.65}} animate={{opacity:1}} transition={{duration:.2}} style={{aspectRatio}} className="relative w-full overflow-hidden rounded-[1.5rem] border border-black/[.07] bg-white shadow-[0_18px_55px_rgba(0,0,0,.07)] sm:rounded-[2rem]">
      <ProductImage src={p.images[img]} alt={p.name} priority sizes="(max-width:1024px) 100vw, 60vw" className="object-contain" onRatio={r=>setRatios(m=>m[img]===r?m:{...m,[img]:r})}/>
      {many&&<span className="absolute bottom-3 right-3 rounded-full bg-white/85 px-3 py-1.5 text-xs font-bold shadow-sm backdrop-blur sm:bottom-4 sm:right-4">{img+1} / {p.images.length}</span>}
     </motion.div>
     {/* Scrolls sideways on phones; wraps on desktop so the last thumbnail is never sliced off. */}
     {many&&<div className="nice-scroll -mx-1 mt-3 flex snap-x snap-mandatory gap-2.5 overflow-x-auto px-1 pb-2 sm:mt-4 sm:flex-wrap sm:snap-none sm:gap-3 sm:overflow-visible sm:pb-0">{p.images.map((x,i)=><button key={`${x}-${i}`} onClick={()=>setImg(i)} aria-label={`${p.name}, şəkil ${i+1}`} aria-pressed={i===img} className={`relative h-[68px] w-[68px] shrink-0 snap-start overflow-hidden rounded-2xl border bg-white transition sm:h-[84px] sm:w-[84px] ${i===img?'border-black ring-1 ring-black':'border-black/10 opacity-60 hover:opacity-100'}`}><ProductImage src={x} alt="" sizes="84px" className="object-contain p-1"/></button>)}</div>}
    </div>
   </div>
   <aside data-info className="min-w-0 lg:sticky lg:top-24">
    <div className="rounded-[1.5rem] border border-black/10 bg-white p-5 shadow-[0_24px_70px_rgba(0,0,0,.08)] sm:rounded-[2rem] sm:p-7 xl:p-8">
     ...
     <div className="mt-4 flex flex-wrap items-end gap-3">{p.newPrice!==null?<><strong className="text-3xl font-black">{p.newPrice} AZN</strong><s className="pb-1 text-lg text-gray-400">{p.price} AZN</s></>:<strong className="text-3xl font-black">{p.price} AZN</strong>}</div>
     <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-y border-black/10 py-4"><span className="text-sm font-semibold">{t.qty}</span><div className="flex items-center rounded-full bg-gray-100 p-1"><button aria-label={t.decrease} onClick={()=>setQty(x=>Math.max(1,x-1))} className="grid h-10 w-10 place-items-center rounded-full bg-white"><Minus size={15}/></button><b className="w-12 text-center">{qty}</b><button aria-label={t.increase} onClick={()=>setQty(x=>x+1)} className="grid h-10 w-10 place-items-center rounded-full bg-black text-white"><Plus size={15}/></button></div></div>
     <button onClick={addQuantity} className="btn btn-dark mt-5 w-full py-4"><ShoppingBag size={19}/>{t.add}</button>
     <div className="mt-3 grid grid-cols-[1fr_auto] gap-3"><button onClick={()=>{addQuantity();router.push('/cart')}} className="btn btn-light"><Zap size={18}/>{t.buyNow}</button><button aria-label={fav?t.removeFav:t.addFav} onClick={()=>toggleFav(p.id)} className={`grid h-12 w-12 place-items-center rounded-full border transition ${fav?'border-black bg-black text-white':'border-gray-200 hover:border-black'}`}><Heart size={19} fill={fav?'currentColor':'none'}/></button></div>
