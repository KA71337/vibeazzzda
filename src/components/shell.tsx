...
import {Grid2X2,Heart,Home,Menu,Search,ShoppingBag,X} from 'lucide-react';
   ...
   <div className={`container transition-all duration-300 ${compact?'max-w-[1240px]':''}`}>
    <div className={`flex items-center justify-between gap-4 border border-black/10 bg-white/95 px-3 shadow-[0_12px_45px_rgba(0,0,0,.08)] backdrop-blur-xl transition-all duration-300 ${compact?'h-16 rounded-[1.25rem]':'h-[4.75rem] rounded-[1.5rem] md:px-5'}`}>
     ...
     <div className="flex items-center gap-0.5">
      <button aria-label={t.searchTitle} onClick={()=>setSearchOpen(true)} className={iconButton}><Search size={19}/></button>
      <Link aria-label={t.favorites} href="/favorites" className={`${iconButton} hidden sm:grid`}><Heart size={19}/></Link>
      <button aria-label={t.cart} onClick={()=>setDrawer(true)} className={`${iconButton} relative`}><ShoppingBag size={19}/>{count>0&&<span className="absolute right-0 top-0 grid h-5 min-w-5 place-items-center rounded-full bg-black px-1 text-[10px] text-white ring-2 ring-white">{count}</span>}</button>
      <button onClick={()=>setMenu(!menu)} aria-label={t.menu} className={`${iconButton} md:hidden`}>{menu?<X size={20}/>:<Menu size={20}/>}</button>
     </div>
    </div>
    {menu&&<div className="mt-2 rounded-[1.5rem] border border-black/10 bg-white p-5 shadow-xl md:hidden"><nav className="grid gap-1">{nav.map(([h,n])=><Link onClick={()=>setMenu(false)} key={h} href={h} className="rounded-xl px-3 py-3 font-semibold hover:bg-gray-50">{n}</Link>)}</nav><div className="mt-4 flex items-center justify-between border-t pt-4"><div className="flex gap-1">{(['az','ru','en'] as const).map(x=><button key={x} onClick={()=>setLang(x)} className={`rounded-full px-3 py-2 text-xs font-bold uppercase ${lang===x?'bg-black text-white':'bg-gray-100'}`}>{x}</button>)}</div><div className="flex"><a aria-label="WhatsApp" href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className={iconButton}><WhatsAppIcon size={18}/></a><a aria-label="TikTok" href={TIKTOK_URL} target="_blank" rel="noopener noreferrer" className={iconButton}><TikTokIcon size={18}/></a><a aria-label="Instagram" href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className={iconButton}><InstagramIcon size={18}/></a></div></div></div>}
   </div>
  </header>
